...
/**
 * PUT /api/merch/club/bundles/[bundleId] — Update a pending bundle (club only)
 */
  ...
  try {
    ...
    if (updateData.title !== undefined) {
      if (
        ...
        updateData.title.trim().length > 200
      ) {
        errors.push({
          field: "title",
    ...
    if (updateData.merchItems !== undefined) {
      if (
        !Array.isArray(updateData.merchItems) ||
        updateData.merchItems.length === 0
      ) {
        errors.push({
          field: "merchItems",
        ...
        updateData.merchItems.forEach((item, i) => {
          if (
            ...
            errors.push({
              field: `merchItems[${i}].name`,
              message: "Merch item name must be between 1 and 100 characters",
            });
          }
          if (
            ...
            errors.push({
              field: `merchItems[${i}].price`,
              message: "Merch item price must be a non-negative number",
            });
          }
          if (item.image !== undefined && typeof item.image !== "string") {
            errors.push({
              field: `merchItems[${i}].image`,
              message: "Merch item image must be a valid URL",
            });
          }
          if (item.sizes !== undefined && !Array.isArray(item.sizes)) {
            errors.push({
              field: `merchItems[${i}].sizes`,
    ...
    if (
      updateData.sizeCharts !== undefined &&
      !Array.isArray(updateData.sizeCharts)
    ) {
      errors.push({
        field: "sizeCharts",
        message: "Size charts must be an array",
      });
    }
    ...
    // Ownership check — club can only edit their own bundles
    if (bundle.club._id.toString() !== user._id.toString()) {
      return NextResponse.json(
        { success: false, message: "Access denied. You can only edit your own bundles." },
        { status: 403 }
      );
    }

    // Approval gate — cannot edit approved bundles
    if (bundle.approvalStatus === "approved") {
      return NextResponse.json(
        { success: false, message: "Cannot edit an approved bundle" },
        { status: 403 }
      );
    }
    ...
    // Re-populate after save so the response includes full references
    await bundle.populate("club", "username clubName");
    await bundle.populate("approvedBy", "username");
    ...
    console.error("Club update bundle error:", error);
    return NextResponse.json(
      {
        success: false,
